...
import hdfcraft.renderers.BitLayerRenderer;
import hdfcraft.renderers.ByteLayerRenderer;
import hdfcraft.renderers.ColourSchemeRenderer;
import hdfcraft.renderers.BiomeRenderer;
import hdfcraft.renderers.LayerRenderer;
import hdfcraft.renderers.NibbleLayerRenderer;
import static hdfcraft.Constants.*;
import static hdfcraft.minecraft.Constants.*;
import hdfcraft.renderers.DimensionAwareRenderer;
...
public final class TileRenderer {
    ...
    public TileRenderer(TileProvider tileProvider, ColourScheme colourScheme, BiomeScheme biomeScheme,
                        CustomBiomeManager customBiomeManager, boolean dry) {
...
//        synchronized (tile) {
        ...
        final boolean _void = layerList.contains(hdfcraft.Void.INSTANCE);
        ...
        if (_void) {
            if (renderBiomes) {
                voidLayers = new Layer[] {hdfcraft.Void.INSTANCE, Biome.INSTANCE};
                voidRenderers = new LayerRenderer[] {hdfcraft.Void.INSTANCE.getRenderer(), biomeRenderer};
            } else {
                voidLayers = new Layer[] {hdfcraft.Void.INSTANCE};
                voidRenderers = new LayerRenderer[] {hdfcraft.Void.INSTANCE.getRenderer()};
            }
        }
        ...
        if (zoom == 0) {
            for (int x = 0; x < TILE_SIZE; x++) {
                for (int y = 0; y < TILE_SIZE; y++) {
                    if (_void && tile.getBitLayerValue(hdfcraft.Void.INSTANCE, x, y)) {
