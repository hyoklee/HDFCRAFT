...
import hdfcraft.PerlinNoise;
import static hdfcraft.Constants.*;
import hdfcraft.Dimension;
import hdfcraft.Tile;
import hdfcraft.AbstractLayerExporter;
import hdfcraft.FirstPassLayerExporter;
import hdfcraft.Resources;
import hdfcraft.ExporterSettings;
    ...
    public void setSettings(ExporterSettings<Resources> settings) {
        ...
        if (resourcesSettings != null) {
            ...
            for (int blockType: resourcesSettings.getBlockTypes()) {
                ...
                for (int i = 0; i < 16; i++) {
                    chances[blockType][i] = PerlinNoise.getLevelForPromillage(
                            Math.min(resourcesSettings.getChance(blockType) * i / 8f, 1000f));
                }
            }
        }
    }
...
//        int[] counts = new int[256];
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                ...
                if (tile.getBitLayerValue(hdfcraft.Void.INSTANCE, localX, localY)) {
                    continue;
                }
